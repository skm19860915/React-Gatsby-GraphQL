...
const FloorPlans2: React.FC = React.memo((props: any) => {
  ...
  let handleBuildingExpand = (title: any) => {
    ...
    let current = {
      AvailableDate:sel.AvailableDate,
      title: sel.title,
      mainImage:
      ...
      unitID: sel.unitID,
      balconySf: sel.associatedFloorPlanTypee[0].balconySf,
      totalGrossUnitSquareFeet:
        sel.associatedFloorPlanTypee[0].totalGrossUnitSquareFeet,
    }
  ...
  const handleFloorplanExpand = (title: any) => {
    ...
    let current = {
      ...
      balconySf: sel.balconySf,
      totalGrossUnitSquareFeet: sel.totalGrossUnitSquareFeet,
    }
    setCurrentUnit(current)
    let sim = floorPlans
      .filter(
    ...
    setUnitVisible(true)
    setSimilarUnits(sim)
    console.log(buildingUnits, leaseMonths)
    let avail = buildingUnits.filter(
      item =>
        ...
        // leaseMonths.filter(
        //   date =>
        //     (new Date(date)).getMonth() === (new Date(item.MadeReadyDate)).getMonth() &&
        //     (new Date(date)).getFullYear() === (new Date(item.MadeReadyDate)).getFullYear()
        // ).length > 0
      // leaseMonths.filter(
      //   date =>
      //     moment(new Date(date)).isSame(item.MadeReadyDate, "month") &&
      //     moment(new Date(date)).isSame(item.MadeReadyDate, "year")
      // ).length > 0
    )
  ...
  const initValues = async () => {
    ...
    let bedrooms = ["0", "1", "2"]
    console.log("url param", search)
    if (Object.keys(search).length > 0 && search.months && search.bedrooms) {
    ...
    console.log("floorplan feeds", res.data)
    res.data.forEach(d => {
      if (d.UnitNumber == 851 || d.UnitNumber == 946 || d.UnitNumber == 948) {
  ...
  return (
    <Home>
      ...
      {/* <Brochure desc="Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostrud exerci tation ullamcorper suscipit lobortis nisl ut aliquip ex ea consequat. "></Brochure> */}
      <Prefooter
...
export default withLocation(FloorPlans2)
